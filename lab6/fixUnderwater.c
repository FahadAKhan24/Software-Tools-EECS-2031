...
THIS LAB RESTORES THE RED PART OF THE PIXEL BY ASSIGNING IT'S VALUE TO
AVAERAGE OF RED,GREEN AND BLUE PARTS OF THE PIXEL.
*/
...
int main(int argc, char *argv[])
{
  ...
  for(i=0;i<height;i++) {
    for(j=0;j<width;j++) {
      ...
      out[i*(3*width)+j*3 + 0] = (r+g+b)/3;
      out[i*(3*width)+j*3 + 1] = g;
      out[i*(3*width)+j*3 + 2] = b;
    }
  }
